...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['save_inventory'])) {
    ...
    foreach ($import_data as $pid => $data) {
        ...
        if ($qty_in > 0) {
            ...
            if ($price_in > 0)
                $sql_update .= ", original_price = $price_in, cost_price = $price_in";
...
if (isset($_GET['stock_filter'])) {
    ...
    switch ($stock_filter) {
        case 'out':
            $sql_prod .= " AND stock = 0";
            break;
        case 'low':
            $sql_prod .= " AND stock > 0 AND stock <= 5";
            break;
        case 'high':
            $sql_prod .= " AND stock > 5";
            break;
    }
}
...
<style>
    ...
    .inp-qty:focus {
        ...
        background: white;
        outline: none;
        box-shadow: 0 0 5px rgba(33, 115, 70, 0.2);
    }
    ...
    <form method="GET" class="filter-row">
        ...
        <div class="filter-group">
            ...
            <select name="stock_filter" class="form-control">
                <option value="all" <?php if ($stock_filter == 'all')
                    echo 'selected'; ?>>Tất cả</option>
                <option value="out" <?php if ($stock_filter == 'out')
                    echo 'selected'; ?>>🔥 Hết hàng (0)</option>
                <option value="low" <?php if ($stock_filter == 'low')
                    echo 'selected'; ?>>⚠️ Sắp hết (1-5)</option>
                <option value="high" <?php if ($stock_filter == 'high')
                    echo 'selected'; ?>>✅ Còn nhiều</option>
            </select>
        </div>
        ...
        <tbody>
            ...
            if (mysqli_num_rows($result_prod) > 0) {
                while ($row = mysqli_fetch_assoc($result_prod)):
                    ...
                    if ($row['stock'] == 0)
                        $stock_style = "background: #f8d7da; color: #721c24; font-weight:bold;";
                    elseif ($row['stock'] <= 5)
                        $stock_style = "background: #fff3cd; color: #856404; font-weight:bold;";
                    ?>
                    <tr>
                        ...
                        <td style="background: #f9fff9;">
                            <input type="number" name="products[<?php echo $row['id']; ?>][qty]" class="inp-qty" placeholder="0"
                                min="0">
                        </td>

                        <td>
                            <input type="number" name="products[<?php echo $row['id']; ?>][price]" class="inp-price"
                                value="<?php echo $row['original_price']; ?>">
                        </td>
                    </tr>
                <?php
                endwhile;
        ...
        <?php while ($h = mysqli_fetch_assoc($result_hist)): ?>
            <div class="hist-item">
                <div style="flex: 1;">
                    ...
                    <span
                        style="color:#999; margin-left:10px; font-size:12px;"><?php echo date('H:i:s d/m', strtotime($h['created_at'])); ?></span>
                </div>
                <div style="color: green; font-weight: bold;">+<?php echo $h['quantity']; ?></div>
            </div>
        <?php endwhile; ?>
        <div style="text-align: center; margin-top: 10px;">
            <a href="inventory_history.php" style="font-size: 13px; text-decoration: none; color: #217346;">Xem tất cả lịch
                sử →</a>
        </div>
    <?php else: ?>
